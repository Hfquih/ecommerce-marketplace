...
export default function AdminSetting(){
    ...
    React.useEffect(()=>{
        const getUserInfo = async () => {
            try{
                const {data} = await apiClient.get(`/auth/${users}` , {headers:{Authorization : `Bearer ${token}`}})
                setUserInfo(data.users)
            }catch(error){
                console.log(`Account detail fetch failed: ${error}`)
            }
        }
        getUserInfo()
    ...
    async function handleForm(e) {
        ...
        try{
            const {data} = await apiClient.patch(`/auth/${users}` , userInfo , {headers: {Authorization : `Bearer ${token}`}})
            showSuccess(data)
        }catch(error){
